    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_PAUSE, ACTION_RESUME -> {
                ...
                if (activeDownloads.isEmpty() && activeIds.isEmpty()) stopSelf(startId)
        ...
        if (foregroundUnavailableIds.contains(id)) {
            ...
            // A stale ACTION_TRACK may belong to a failed provisional id while another download
            // is already tracked (or is still waiting for its own start command). Stopping here
            // would drop foreground support for that unrelated download.
            if (activeDownloads.isEmpty() && activeIds.isEmpty()) {
                stopSelf(startId)
        ...
        if (finishedBeforeStart.remove(id)) {
            ...
            if (activeDownloads.isEmpty()) {
                ...
                if (activeIds.isEmpty()) stopSelf(startId)
            } else {
                promote()
            }
            return START_NOT_STICKY
