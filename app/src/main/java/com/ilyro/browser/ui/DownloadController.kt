...
internal class DownloadController(
    ...
    fun enqueueNavigationWithSession(
        ...
        if (!networkAllowed(allowMetered)) return false
    ...
    fun resolveMediaQualities(
        ...
        }, "ILYRO-media-quality-resolver").also { it.start() }
        return {
            cancelled.set(true)
            worker.interrupt()
        }
    ...
    fun enqueueHlsDownload(
        ...
        Thread({
            ...
            try {
                ...
                while (true) {
                    val fetched = fetchHlsText(playlistUrl, referrer, isPrivate, allowMetered)
        ...
        Thread({
            ...
            try {
                ...
                if (cancelled) {
                    ...
                    liveTransfers.computeIfPresent(id) { _, live ->
                        live.copy(speedBytesPerSecond = 0L)
                    }
    ...
    private fun resume(id: Long): Boolean {
        ...
        if (record.isHls &&
            ...
            val pausedItem = DownloadUiItem(
                ...
                downloadedBytes = mediaSize(record.localUri),
                totalBytes = record.expectedBytes
            )
            remove(pausedItem)
            return enqueueHlsDownload(
                url = record.sourceUrl,
                suggestedTitle = record.fileName.substringBeforeLast('.', record.fileName),
    ...
    fun retry(item: DownloadUiItem, allowMetered: Boolean): DownloadRecord? {
        ...
        remove(item)
        if (source.isHls) {
            val started = enqueueHlsDownload(
                url = source.sourceUrl,
                suggestedTitle = source.fileName.substringBeforeLast('.', source.fileName),
                referrer = source.referrer,
                isPrivate = source.isPrivate,
                allowMetered = allowMetered
            )
            return if (started) source.copy(directState = DIRECT_RUNNING, allowMetered = allowMetered) else null
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            val geckoRetry = enqueueGeckoFetch(
                url = source.sourceUrl,
    ...
    private fun retryAfterDirectFailure(
        ...
        if (scheme != "http" && scheme != "https") {
            addFailedRecord(
                failedId, url, fileName, mimeType, expectedBytes, referrer, isPrivate, allowMetered
            )
            return
        }
                ...
                for (index in 0 until array.length()) {
                    ...
                    add(
                        DownloadRecord(
                            ...
                            allowMetered = item.optBoolean("allowMetered", true),
                            isHls = item.optBoolean("isHls", false)
                        )
                    )
                }
        ...
        records.take(MAX_RECORDS).forEach { record ->
            array.put(
                JSONObject()
                    ...
                    .put("allowMetered", record.allowMetered)
                    .put("isHls", record.isHls)
            )
        }
